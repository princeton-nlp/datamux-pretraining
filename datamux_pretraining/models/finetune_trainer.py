...
class TensorBoardCallbackv2(TensorBoardCallback):
    ...
    def on_train_begin(self, args, state, control, **kwargs):
        ...
        if self.tb_writer is not None:
            ...
            if hasattr(self.tb_writer, "add_hparams"):
                if "model" in kwargs:
                    model = kwargs["model"]
                    hparams = dict(args.to_sanitized_dict(), **model.config.to_dict())
                else:
                    hparams = dict(args.to_sanitized_dict())
...
class FinetuneTrainer(Trainer):
    def __init__(
        ...
        eval_dataset_unpruned: Optional[Dataset] = None,
        ...
        self.eval_dataset_unpruned = copy.deepcopy(eval_dataset)
        if is_datasets_available():
            if isinstance(
        ...
        self.add_callback(TensorBoardCallbackv2)
        self.pop_callback(WandbCallback)
        self.add_callback(WandbCallbackThreadFix)
        # very last
    ...
    def evaluate(
        ...
        if self.post_process_function is not None and self.compute_metrics is not None:
            eval_examples = (
                self.eval_examples if self.eval_examples is not None else None
            )
            eval_dataset = (
                eval_dataset if eval_dataset is not None else self.eval_dataset_unpruned
            )
            ...
            self.log(metrics, use_global_step=False)
        else:
        ...
        ensemble_calc = False 
        if ensemble_calc:
            output_ensemble = self.prediction_loop_ensemble(
                eval_dataloader,
                description="Evaluation",
                # No point gathering the predictions if there are no metrics, otherwise we defer to
                # self.args.prediction_loss_only
                prediction_loss_only=prediction_loss_only,
                ignore_keys=ignore_keys,
                metric_key_prefix=metric_key_prefix + "_ensemble",
            )
            output.metrics.update(output_ensemble.metrics)
        if speed_metrics:
            # train_dataloader = self.get_train_dataloader()
        ...
        # get N samples from the eval dataset
    ...
    def prediction_loop_ensemble(
        ...
        for step, inputs in enumerate(dataloader):
            # repeat each instance N times
            instance_map = torch.randperm(inputs["input_ids"].shape[0] * self.model.config.num_instances, device=self.args.device)
            for k, v in inputs.items():
                if isinstance(v, torch.Tensor) and k != "labels":
                    v = torch.repeat_interleave(
                        v, self.model.config.num_instances, dim=0
                    )
                    inputs[k] = v[instance_map]
            labels = inputs.pop("labels")
            # use the instance map for ensemble label calculation
            (_, logits, _, _, _, _, _,) = self.prediction_step(
                model, inputs, prediction_loss_only, ignore_keys=ignore_keys,
            )
            logits_copy = logits.clone()
            logits_copy[instance_map] = logits
            logits = logits_copy
            if logits is not None:
                # average logits across the N copies
                logits = logits.view(
                    -1, self.model.config.num_instances, logits.shape[-1]
                )
            ...
            if has_labels:
                ...
                if isinstance(outputs, dict):
                    ...
                    logits = (
                        ...
                        else tuple([outputs["start_logits"], outputs["end_logits"]])
                    )
                else:
                    logits = outputs[1:]
